...
(16, '高雄旗山地區PRET飲品店創新經營可行方案', '韓慧林', '論文類', '高雄旗山地區PRET飲品店創新經營可行方案', '楊?博', 'A9728307', '2013-05-13'),
...
(45, '資料探勘實務性操作應用支援向量機與時間序列預測證?交易市場分析之研究', '王貴民', '系統類', '本研究針對目前國外上市上櫃公司的股票做完整的分析研究，用資料探勘(Data Mining)中的時間序列和技術指標的觀念來建立一個決策分析系統，並加上學術理論為佐證，在本研究中，時間序列本身便是專門來研究預測且連續數值的規律，而我們找了國外一家上市公司(PG)，針對他們的市場交易價格來預測未來的漲跌。', '吳佳玲', 'A9228018', '2013-05-30'),
...
(51, '利用資料採礦技術分析就業市場 需求於資管教育之研究', '王貴民', '論文類', '利用資料採礦技術，分析出業界對各項專長的需求狀況與趨勢。在此，我們以資訊相關科系為例，針對不同的職業類別做分析，依照學歷、語文、電腦專長等條件分析出各種不同的需求資訊，並可產生專長需求排行榜。', '侯?宇', 'A9228023', '2013-05-30'),
...
(67, '霸王別姬', '羅建銘', '模擬類', '近幾年來，華人在世界舞台上表現亮眼，不論在政治、經濟、文化等方面都佔有一席之地，因而帶動世界一股「中國風」。以電影為例：李安的「臥虎藏龍」，張藝謀的「英雄」、「十面埋伏」，成龍的「神話」以及李連杰「霍元甲」等。這些成績斐然的電影應用的都是中國五千年歷史所珍藏的文化資產，中國題材創造無限地商業與藝術價值。政治與經濟的侵襲是有形的，文化造成的?移默化為無形的，我認為「中國風」所帶動的潮流不止於現狀，未來「中國文化」將蔚為風潮主導世界時尚潮流。', '黃雅薇', 'A9128015', '2013-05-30'),
...
(104, '校友問卷調查系統', '葉期財', '系統類', '??是從社會、學校、系所、在校學生或是校友角??看，校友對學校和系所實?的增進都有?可或缺的重要性。校友是本校各系所的重要資源與人脈，關心校友、重視並努?開發校友資源，是增強學校各系所實?的重要策?。各?各業的校友也是銜接各系所與產業、企業重要的橋樑之一，因為他們是知?密集、資訊?敏、與各系所有特殊感情的群體。近幾??，?續有校友把產業所碰到的問題帶到學校，得到各系所教授的專業協助與解決。他們?僅因此為各系所帶?資源，也讓各系所發揮人才優勢，對產業有所貢獻，也對社會產生影響?。透過產學合作可以幫企業解決問題，也能夠讓學校知道社會動向調整教學方針。', '王聖淵', 'A9428338', '2013-06-04'),
...
(114, '線上遊戲介面對數位學習成效之影響 -以大學新生輔導為例', '葉期財', '模擬類', '本研究即以「多人?線遊戲模式的?位學習」為題，探討此一?位學習模式的特性以及所能達成的學習成效。研究中除?實際建構出一套名為「大學生了沒」的有關學校提供給新生的相關知?學習遊戲之外，並針對一部份內容實際進?學習效果的驗證。', '楊子瑩', 'A9428405', '2013-06-04'),
...
(120, '電腦急救站 -防毒救援', '張良政', '系統類', '做出一個簡單的網站?使用者方便又快速就可以解決中毒的煩惱，且中毒的徵兆也會一一條列，讓受害者知道電腦已受到病毒的威脅，是量身打造給一般人使用，讓他們知道現在是處於什麼狀況，也是免付費的狀況下給一般人使用，能帶來完整的幫助性以及便利的使用。首先會先讓受害者輸入他的中毒症狀依症狀查詢是哪種病毒，再來告知受害者該如何操作才能讓電腦恢復以往，因此對電腦概念一竅不通者，使用這頁面可以很快找出主要原因，提供一般人可有方面快速的解決。', '王書維', 'A9528310', '2013-06-04'),
...
(124, '企業商品的品質績效決策系統', '洪清文', '論文類', '本研究探討產品壽命在冪次分配族(Power distribution family)的假設下，透過最小平方估計法，發展出一個新的假設檢定程序作為產品的績效評估，此假設檢定程序是即時商業智慧中一個企業產品的品質績效決策系統，能處理非常態且設限的產品壽命資料。透過此品質績效決策系統，企業可以?解所提供的產品壽命是否達到規格要求，並提升產品品質，以?能符合顧客的需求。 ', '陳菁甫', 'A9528334', '2013-06-06'),
...
(144, '產銷履歷系統結合RFID-應用於生鮮食品以海鱺魚為例', '陳仁祥', '系統類', '本研究建置漁業生產履歷系統，讓每位生產者擁有自己的帳號來管理自己產品的資訊，將此建構的產品資訊傳至Web Server。讓消費者利用產銷履歷號碼來將產銷履歷資訊由Data Base呼叫至網頁，消費者即可從網頁得知產品從加工廠經由運輸至餐廳業者所有的行動記錄，產銷履歷對業者可視為產品品質把關的動作，餐廳業者可藉由消費者的回應來進行貨品來源調查及運輸狀況調查。', '陳?志', 'A9628321', '2013-06-06'),
...
(171, 'RFID應用於校園安全管理-以幼稚園為例', '葉期財', '系統類', '以校園安全而言，我們將結合RFID科技與Web cam，使家長和幼稚園能夠第一時間掌握孩童上下學的資訊。每一位孩童都擁有一張獨有的RFID電子?別晶片卡，讓幼稚園孩童隨身攜帶。當孩童進入或者?開學區時，設?在學區門口的RFID?寫器一接收到晶片卡的無線訊號就會發出訊息到學校的主機上，同時Web cam也會即時擷取影像，如此一來不只可以紀?孩童的上下學的時間，另一方面系統會自動呼叫寄送簡訊的服務，然後傳送到各個家長的手機與e-mail信箱通知家長，訊息內容包含孩童到達學區或者?開學區的時間與影像，讓家長可以即時掌握到孩童的安全資訊；同時，校方也能掌握孩童到校的動態，共同組成一個?安全的電子守護網。', '呂韋德', 'A9328027', '2013-06-06'),
